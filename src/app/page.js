...
function NewBidModal({ onClose, onSave, userEmail }) {
  const [f, setF] = useState({ name: '', client: '', due: '', walk: '', walkLoc: '', owner: 'Craig', ownerEmail: userEmail, rfpUrl: '', notes: '' });
  ...
  const validate = () => {
    const nextErrors = {};
    if (!f.name.trim()) nextErrors.name = 'Bid name is required.';
    if (!f.client.trim()) nextErrors.client = 'Client is required.';
    if (!f.due) nextErrors.due = 'Due date is required.';
    if (!f.owner.trim()) nextErrors.owner = 'Owner is required.';
    if (!isValidUrl(f.rfpUrl)) nextErrors.rfpUrl = 'Enter a valid URL starting with http:// or https://';
      ...
      <div className="w-full sm:w-[480px] bg-white h-full overflow-auto shadow-xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-6 py-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 mb-3.5">
            {[['Bid Name *', 'name', 'text', 'e.g. Custodial Services'], ['Client *', 'client', 'text', 'e.g. Town of Weymouth'], ['Due Date *', 'due', 'date', ''], ['Owner *', 'owner', 'select', ''], ['Walk Date', 'walk', 'datetime-local', ''], ['Walk Location', 'walkLoc', 'text', 'Address'], ['RFP URL', 'rfpUrl', 'url', 'https://...']].map(([label, key, type, ph]) => (
              <div key={key}>
                <label className="block text-[12px] font-semibold text-slate-500 mb-1.5">{label}</label>
                {type === 'select' ? (
                  <select value={f[key]} onChange={e => { up(key, e.target.value); if (errors[key]) setErrors((prev) => ({ ...prev, [key]: '' })); }} className={`w-full px-3 py-2.5 rounded-lg border text-[13px] text-slate-700 outline-none bg-slate-50 ${errors[key] ? 'border-red-300' : 'border-slate-200'}`}>
                    <option>Craig</option><option>Craig Jr.</option>
                  </select>
                ) : (
                  <input type={type} placeholder={ph} value={f[key]} onChange={e => { up(key, e.target.value); if (errors[key]) setErrors((prev) => ({ ...prev, [key]: '' })); }} className={`w-full px-3 py-2.5 rounded-lg border text-[13px] text-slate-700 outline-none bg-slate-50 ${errors[key] ? 'border-red-300' : 'border-slate-200'}`} />
                )}
                {errors[key] && <p className="text-[11px] text-red-500 mt-1">{errors[key]}</p>}
                {key === 'rfpUrl' && !errors[key] && <p className="text-[11px] text-slate-400 mt-1">Paste a direct RFP document or portal link.</p>}
              </div>
            ))}
          </div>
          <div>
          ...
          {errors.form && <div className="mt-4 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-[12px] text-red-600">{errors.form}</div>}
          <div className="flex gap-2.5 justify-end mt-5">
            ...
            <button onClick={handleSubmit} disabled={saving} className="px-4 py-2.5 rounded-lg bg-navy-500 text-white text-[13px] font-bold hover:bg-navy-600 disabled:opacity-50">
